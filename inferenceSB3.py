...
with open(file_path, 'w') as file:
    file.write("")
# Run inference
for _ in range(1000):  # Adjust the number of episodes you want to run inference on
    obs, _ = env.reset()
    done = False
    trunc = False
    # while not done:
    ...
    while not done: # not done:
        ...
        # print('reward', reward)
        # print(info)
        random_action, _ = model.predict(obs, deterministic=True)
    ...
    with open("output.txt", 'a') as file:
        file.write("Episode reward:" + str(env.reward) + ", Episode length: " + str(env.episode_length) + "\n")
    print(f"Episode reward: {env.reward}, Episode length: {env.episode_length}")

    if env.episode_length < 70:
        break
